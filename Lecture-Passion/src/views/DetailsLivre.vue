...
function OnSubmit() {
  ...
  if (commentaires.value) {
    moyenneNote.value =
      (commentaires.value.reduce((sum, note) => sum + note.comAppreciation, 0) + note.value) /
      (commentaires.value.length + 1);
  } else {
    moyenneNote.value = note.value;
  }
  updateLivre(id.value, {
    ouvMoyenneAppreciation: Math.round(moyenneNote.value),
  });
}
  ...
  <div v-if="livre">
    <div class="detail-livre">
      ...
      <div class="detail-livre-text">
        ...
        <p>
          Auteur : {{ livre.t_ecrivain.ecrPrenom }}
          {{ livre.t_ecrivain.ecrNom }}
        </p>
